...
var page_msg = "";//页面上获取到的msg
var back_msg = "";//接口返回的msg
var flag6 = false;//两个msg是否一致的标识值
...
//点击获取短信验证码 获取短信验证码
$(".get_msg_btn").on("click",function(){
	var phone = $(".username_all input").val()
	$.ajax({
		type:"get",
		url:"http://console.fangmingwei.com:3000/user/msgcode",
		async:true,
		cache:false,
		timeout:3000,
		data:{
...
//短信验证码的填入
$(".msg_code_all input").on("input",function(){
	page_msg = $(".msg_code_all input").val();
	if(page_msg == back_msg){
		flag6 = true;
	}else{
		flag6 = false;
	}
...
$(".main_middle_zhuce_btn span").click(function(){
	if(flag6){
		if(flag1&&flag2&&flag3&&flag4&&flag5){
			$.ajax({
				type:"post",
				url:"http://console.fangmingwei.com:3000/user/register",
				async:true,
				data:{
					msg: page_msg,
					password:$(".password_all input").val(),
					phone:$(".username_all input").val()
				},
				success:function(data){
					console.log(data);
					if(data.code == 1){
